...
public class WestCoastDrive extends Subsystem {
    ...
    public void initDrive() {
        ...
        leftMaster.setSmartCurrentLimit(RobotMap.SPARK_LIMIT);
        leftFront.setSmartCurrentLimit(RobotMap.SPARK_LIMIT);
        leftRear.setSmartCurrentLimit(RobotMap.SPARK_LIMIT);
        rightMaster.setSmartCurrentLimit(RobotMap.SPARK_LIMIT);
        rightFront.setSmartCurrentLimit(RobotMap.SPARK_LIMIT);
        rightRear.setSmartCurrentLimit(RobotMap.SPARK_LIMIT);
    ...
    public void setToCoast() {
        ...
        leftMaster.setClosedLoopRampRate(RobotMap.RAMP_RATE);
        rightMaster.setClosedLoopRampRate(RobotMap.RAMP_RATE);
    }
    ...
    public double getSpeed(Mode mode, Controller driver) {
        ...
        case ARCADE:
        case TANK:
            return purell(HelperFunctions.deadzone(driver.getY(RobotMap.SPEED_HAND)));
        case GTA:
            return purell(HelperFunctions.deadzone(
                    driver.getTriggerAxis(RobotMap.RIGHT_HAND) - driver.getTriggerAxis(RobotMap.LEFT_HAND)));
        default:
    ...
    public double getRotation(Mode mode, Controller driver) {
        switch (mode) {
        case CURVATURE:
            return purell(HelperFunctions.deadzone(driver.getX(RobotMap.ROT_HAND)));
        case ARCADE:
        case TANK:
            return purell(HelperFunctions.deadzone(driver.getY(RobotMap.ROT_HAND)));
        case GTA:
            return purell(HelperFunctions.deadzone(driver.getX(RobotMap.ROT_HAND)));
        default:
            // curvature drive default
    ...
    public double purell(double speed) {
    ...
    public double[] getCurrentArray() {
        double[] currentArray = new double[6];
        currentArray[0] = rightMaster.getOutputCurrent(); currentArray[1] = rightRear.getOutputCurrent(); currentArray[2] = rightFront.getOutputCurrent();
        currentArray[3] = leftMaster.getOutputCurrent(); currentArray[4] = leftRear.getOutputCurrent(); currentArray[5] = leftFront.getOutputCurrent();
        return currentArray;
    }
}
